...
public class Task04 extends AbstractTestNGSpringContextTests
{
    ...
    {
        List<Product> found = em
            .createQuery("SELECT p FROM Product p WHERE p.categories IS NOT EMPTY", Product.class)
            .getResultList();
    ...
    {
        List<Product> found = em
            .createQuery("SELECT p FROM Product p JOIN FETCH p.categories c", Product.class)
            .getResultList();
    ...
    {
        List<Object[]> found = em
            .createQuery("SELECT p.color, COUNT(p) FROM Product p GROUP BY p.color ORDER BY p.color", Object[].class)
            .getResultList();
    ...
    public void findProductsAddedYesterday()
    {
        ...
        List<Product> products = em
            .createQuery("SELECT p FROM Product p WHERE p.addedDate = :date", Product.class)
    ...
    public void leftJoinCagesWithPets()
    {
        ...
        List<ProductAndCategory> productAndCategory = em
            .createQuery("SELECT NEW cz.fi.muni.pa165.tasks.ProductAndCategory(p, c) FROM Product p LEFT JOIN p.categories c ON c.name = :kitchenName ORDER BY p.name, c.name", ProductAndCategory.class)
            .setParameter("kitchenName", "Kitchen")
            .getResultList();
